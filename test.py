...
"""Test for kineticstoolkit_pushrimkinetics."""
...
def test_read_smartwheel():
    ...
    kinetics_csv = pk.read_smartwheel(
        "tutorial_data/pushrimkinetics_propulsion.csv"
    )

    kinetics_txt = pk.read_smartwheel(
        "tutorial_data/pushrimkinetics_propulsion.txt"
    )
...
def test_remove_offsets():
    ...
    kinetics = pk.read_smartwheel(
        "tutorial_data/pushrimkinetics_offsets_propulsion.csv"
    )

    baseline = pk.read_smartwheel(
        "tutorial_data//pushrimkinetics_offsets_baseline.csv"
    )
    ...
    assert np.all(np.abs(no_offsets1.data["Forces"] - no_offsets2.data["Forces"]) < 1)
